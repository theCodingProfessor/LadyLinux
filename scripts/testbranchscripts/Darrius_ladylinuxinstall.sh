...
ExecStart=$VENV_DIR/bin/uvicorn api_layer.app:app --host 0.0.0.0 --port $API_PORT
Restart=on-failure
RestartSec=10
StandardOutput=journal
StandardError=journal
...
if ! pgrep -f "uvicorn api_layer.app:app" >/dev/null 2>&1; then
...
start_and_validate() {
    ...
    systemctl start "$API_SERVICE" || {
        ...
        sudo -u "$SERVICE_USER" bash -c \
            "cd '$APP_DIR' && nohup '$VENV_DIR/bin/uvicorn' api_layer.app:app \
